...
public class Game : GameBase, ISceneRendererContext, IGameSettingsService
{
    ...
    /// <summary> Static event that will be fired when a game is initialized </summary>
    public static event EventHandler GameStarted;

    /// <summary> Static event that will be fired when a game is destroyed </summary>
    ...
    public IFontFactory Font
    {
        get
        {
            if (gameFontSystem.FontSystem == null) throw new InvalidOperationException("The font system is not initialized yet");
    ...
    protected override void Destroy()
    {
        ...
        if (logListener != null) GlobalLogger.GlobalMessageLogged -= logListener;
    }
        ...
        if (Context.InitializeDatabase)
        {
            ...
            if (Content.Exists(GameSettings.AssetUrl))
            {
                ...
                if (renderingSettings.DefaultGraphicsProfile > 0)
                {
                    ...
                    if (!deviceManager.ShaderProfile.HasValue) deviceManager.ShaderProfile = renderingSettings.DefaultGraphicsProfile;
                }
    ...
    protected override void Initialize()
    {
        ...
        if (Settings != null) EffectSystem.SetCompilationMode(Settings.CompilationMode);

        GameSystems.Add(EffectSystem);

        if (Settings != null) Streaming.SetStreamingSettings(Settings.Configurations.Get<StreamingSettings>());
    ...
    private void DestroyAssetDatabase()
    {
        ...
        {
            if (Services.GetService<IDatabaseFileProviderService>() is DatabaseFileProviderService dbfp) dbfp.FileProvider = null;
